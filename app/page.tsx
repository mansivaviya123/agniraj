...
export default function Home() {
 ...
 <section id="solutions" className="section solutions-section"><div className="container"><div className="section-heading" data-reveal><p className="eyebrow">Our solutions</p><h2>Explore protection built around your risk.</h2><p className="section-intro">Select a solution to see how Agniraj can support your facility.</p></div><div className="solution-explorer solution-explorer-desktop" data-reveal><div className="solution-tabs" role="tablist" aria-label="Agniraj solutions" onKeyDown={event => { if (event.key === 'ArrowDown' || event.key === 'ArrowRight') chooseSolution((selected + 1) % solutions.length); if (event.key === 'ArrowUp' || event.key === 'ArrowLeft') chooseSolution((selected - 1 + solutions.length) % solutions.length); }}>{solutions.map((s,i) => <button key={s.name} id={`solution-tab-${i}`} role="tab" aria-selected={selected === i} aria-controls="solution-panel" tabIndex={selected === i ? 0 : -1} className={`solution-tab ${selected === i ? 'is-active' : ''}`} onClick={() => chooseSolution(i)}><span className="solution-index">0{i+1}</span><span className="solution-tab-copy"><strong>{s.name}</strong><small>{s.description}</small></span><ArrowRight size={21} /></button>)}</div><article id="solution-panel" key={selected} role="tabpanel" aria-labelledby={`solution-tab-${selected}`} className="solution-panel"><div className="solution-visual"><img src={`/images/${service.image}`} alt={`${service.name} by Agniraj`} /><span className="solution-watermark">0{selected+1}</span></div><div className="solution-detail"><div><p className="eyebrow">Selected solution</p><h3>{service.name}</h3><p>{service.description}</p></div><ul>{service.items.map(item => <li key={item}><Check size={18} />{item}</li>)}</ul><button className="button solution-cta" onClick={talk}>Discuss this solution <ArrowRight size={19} /></button></div></article></div><div className="mobile-solutions" data-reveal>{solutions.map((s,i) => <div className={`mobile-solution-item ${selected === i ? 'is-active' : ''}`} key={s.name}><button type="button" className="mobile-solution-trigger" aria-expanded={selected === i} aria-controls={`mobile-solution-panel-${i}`} onClick={() => chooseSolution(i)}><span>0{i+1}</span><strong>{s.name}</strong><ArrowRight size={21} /></button>{selected === i && <article id={`mobile-solution-panel-${i}`} className="mobile-solution-panel"><div className="mobile-solution-image"><img src={`/images/${s.image}`} alt={`${s.name} by Agniraj`} /><span>0{i+1}</span></div><div className="mobile-solution-detail"><p>{s.description}</p><ul>{s.items.map(item => <li key={item}><Check size={17} />{item}</li>)}</ul><button className="button" onClick={talk}>Discuss this solution <ArrowRight size={18} /></button></div></article>}</div>)}</div></div></section>
